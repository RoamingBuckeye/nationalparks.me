...
**Still to provision:** the remaining environment variables (`APP_*`, `MAIL_MAILER=ses` + SES creds, `CACHE_STORE`/`SESSION_DRIVER=database`, `LOG_CHANNEL=stderr`, `NPS_API_KEY`), the GitHub repo authorization for the org, SES domain/production-access, and the first deploy.
