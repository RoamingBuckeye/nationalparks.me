...
- **Database:** serverless Postgres cluster `nationalparks` — id `frosty-snow-41947171` (`neon_serverless_postgres_18`), region `us-east-2`, scale-to-zero (0.25–1 CU, 300s suspend, 7-day PITR); schema `main`. **Linked to the `production` environment** (Cloud injects `DB_*`).
- **Object storage:** bucket `nationalparks-photos` — id `fls-a230aa7d-0d5a-4644-9bbb-525dabee547f`, private, Cloudflare R2-backed, read/write key. **Linked to the `production` environment** (Cloud injects the S3 credentials).
- **Managed Queue:** `default` — id `inst-a230b003-1529-48c4-900c-71c7b54b307f`, `mq-pro-256mb`, custom scaling (max 3 replicas), scales to zero; worker drains the `cloud` connection. Cloud sets `QUEUE_CONNECTION=cloud` on deploy.
...
**Environment configured (2026-07-05):** `APP_NAME/ENV/DEBUG/URL`, `CACHE_STORE`/`SESSION_DRIVER=database`, `LOG_CHANNEL=stderr`, `MAIL_MAILER=ses` + `MAIL_FROM_*` + `SES_REGION`, and `NPS_API_KEY` are set (`APP_KEY` is auto-generated by Cloud). Set via `POST /environments/{env}/variables` with `method:"set"` (upsert). `APP_URL` is the vanity domain `nationalparksme-production-kukmiv.laravel.cloud` until DNS cutover. **Deploys are green as of #68** — the attached bucket required `league/flysystem-aws-s3-v3` (that was the #67 failure).

**Still to provision:** `SES_KEY` + `SES_SECRET` (create an SES IAM user, verify the `nationalparks.me` domain, request production access to exit the SES sandbox), then the custom-domain DNS cutover to `nationalparks.me` (update `APP_URL`, which also fixes the passkey RP ID) before opening signups.
